...
def script_wine():
    ...
    for index, text in enumerate(data.sentences[0:10]):
        ...
        for sentence in text:
            ...
            for wordPair in wordPairs:
                #word1 = Node('Feature', word=wordPair[0])
                #database.graph.merge_one(word1)
                word1 = database.graph.merge_one('Feature', 'word', wordPair[0])
                #word2 = Node('Feature', word=wordPair[1])
                #database.graph.merge_one(word2)
                ...
                #existRelationWord1Word2 = sum(1 for x in database.graph.match(start_node=word1, rel_type='followed by', end_node=word2))
                #if existRelationWord1Word2 >=1:
                #    print 'Increase weight'
                #    rel.properties['weight'] = rel.properties['weight'] + 1
                #    rel.push()
                #else:
                #    database.graph.create(Relationship(word1, 'followed by', word2, weight=1))
                #if existRelationDocWord1 >= 1:
